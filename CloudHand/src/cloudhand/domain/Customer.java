...
public class Customer{
    ...
    public void setBirthday(Date bDate) throws CHException {
        if ((bDate == null)
                || (bDate != null && bDate.before(new Date()))) {
            birthday = bDate;
        } else {
            System.err.println("出生日期必須小於現在!");
            throw new CHException("出生日期必須小於現在!");
        }
    }

    public void setBirthday(String bDate) throws CHException { //yyyy-MM-dd
        if (bDate != null && (bDate = bDate.trim()).length() > 0) {
            ...
            try {
                ...
                throw new CHException("客戶生日日期格式不正確: " + bDate);
            }
...
//            }
    ...
    public void setBirthday(int year, int month, int day) throws CHException {
...
//                        System.out.println("身分證號首碼無法轉換: " + firstChar));
     ...
     */
    public void setId(String id) throws CHException {
        if (id != null && checkId(id = id.trim())) {
            this.id = id;
        } else {
            System.err.println("身分證號不正確");
            throw new CHException("身分證號不正確");
        }
    }
     ...
     */
    public void setPassword(String password) throws CHException {
        if (password != null
            ...
            throw new CHException("密碼長度必須在6~20之間");
        }
    }
     ...
     */
    public void setName(String name) throws CHException {
        if (name != null && (name = name.trim()).length() > 0) {
            this.name = name;
        } else {
            System.err.println("姓名為必要欄位");
            throw new CHException("姓名為必要欄位");
        }
    }
     ...
     */
    public void setEmail(String email) throws CHException {
        if (email != null
                && (email = email.trim()).matches("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")) {
            this.email = email;
        } else {
            System.err.println("email格式不正確");
            throw new CHException("email格式不正確");
        }
    }
     ...
     */
    public void setGender(char gender) throws CHException {
        if (gender == MALE || gender == FEMALE) {
            this.gender = gender;
        } else {
            System.err.println("性別資料不正確");
            throw new CHException("性別資料不正確");
        }
    }
